...
# ---------------------------------------------------------------------------
# Upstox API rate limits (user-specified):
#   50 requests / second
#   500 requests / minute
#   2000 requests / 30 minutes
#
# We target 25 req/s sustained (50% headroom on the per-second limit).
# Over 60s that's 1500 req/min — but we're capped to 500/min, so we need
# a dual-bucket limiter.  In practice the per-minute limit dominates:
#   500 req / 60s ≈ 8.3 req/s sustained, well under the per-second cap.
#
# We use a simple token-bucket approach with two buckets:
#   Bucket 1: 50 tokens, refill 50/s   (per-second cap)
#   Bucket 2: 500 tokens, refill 500/60s ≈ 8.33/s  (per-minute cap)
# A request must obtain a token from *both* buckets.
#
# Max concurrency for the thread pool — enough to keep the pipeline full
...
class _TokenBucket:
    ...
    def acquire(self) -> None:
        ...
        while True:
            ...
            # Wait a short interval before retrying (avoids spin-locking)
            time.sleep(0.02)


# Two rate-limit buckets — a request must pass through both.
_bucket_per_second = _TokenBucket(capacity=50, refill_rate=50.0)
_bucket_per_minute = _TokenBucket(capacity=500, refill_rate=500.0 / 60.0)


def _rate_limit_acquire() -> None:
    """Acquire a token from both buckets before making an API call."""
    _bucket_per_second.acquire()
    _bucket_per_minute.acquire()


# ---------------------------------------------------------------------------
# Reusable API client (module-level, thread-safe via SDK's urllib3 pool)
# ---------------------------------------------------------------------------

_api_lock = threading.Lock()
_api_instance = None  # lazily initialised


def _get_api_instance():
    """
    Return a shared HistoryV3Api instance.  Created once and reused so that
    the underlying urllib3 connection pool is shared across threads/calls.
    """
    ...
    with _api_lock:
        ...
        # Resolve access token once
        try:
            from broker.upstox.auth import auth_manager
            token = auth_manager.get_valid_token() or "dummy_token"
        except Exception:
            token = "dummy_token"
        ...
        _api_instance = upstox_client.HistoryV3Api(
...
def _resolve_date_range(
    ...
    if from_date:
        ...
        start = _period_to_from_date(period, end)
    else:
        raise ValueError(
...
def _period_to_from_date(period: str, from_date: date) -> date:
    """
    Convert a human-readable period string to a start date.

    Supported formats: "<N>days", "<N>weeks", "<N>months", "<N>years"
    (case-insensitive, optional space).  E.g. "3months", "90 days", "2Years".
    """
    import re
    m = re.fullmatch(r"(\d+)\s*(days?|weeks?|months?|years?)", period.strip().lower())
    if not m:
        raise ValueError(
            f"Cannot parse period '{period}'.  "
            "Use format: '3months', '90days', '2years', '4weeks'."
        )
    ...
    if unit in ("years", "year"):
        try:
            ...
            return from_date.replace(year=from_date.year - n, day=28)
...
def _cache_is_current(latest_cached: date, storage_unit: str) -> bool:
    ...
    - For minute data: cache is current if latest_cached >= last_trading_day
      AND (we are past 15:35 IST today OR last_trading_day < today).
    ...
    Timezone fix:
    Use timezone-aware IST datetimes on both sides of the comparison.
    This avoids naive-vs-aware drift on cloud VMs running in UTC and
    preserves the original cache-current logic.
    """
    ...
    # and time is before 15:35 IST), treat yesterday as the last complete day.
        ...
        if now_ist.weekday() < 5 and now_ist < market_close_ist:
            # Market may still be open — last *complete* day is yesterday
            last_complete_day = _last_trading_day(now_ist.date() - timedelta(days=1))
        else:
...
def _get_latest_cached_date(trading_symbol: str, storage_unit: str) -> Optional[date]:
    ...
    Optimisation: uses PyArrow Parquet metadata (row-group column statistics)
    to read the max timestamp without loading any row data.  Falls back to
    a minimal pandas read if PyArrow metadata is unavailable.
    """
    if storage_unit == "minutes":
        ...
        return _max_date_from_parquet(last_file)

    elif storage_unit == "days":
        day_file = DAY_DIR / f"{trading_symbol}.parquet"
        if not day_file.exists():
            return None
        return _max_date_from_parquet(day_file)

    return None


def _max_date_from_parquet(file_path: Path) -> Optional[date]:
    """
    Extract the maximum timestamp date from a Parquet file as cheaply as
    possible.  Uses PyArrow row-group statistics (zero row reads) when
    available; falls back to a minimal pandas column read otherwise.
    """
    # Fast path: PyArrow metadata — no data read at all
    if pq is not None:
        try:
            pf = pq.ParquetFile(file_path)
            # Find the timestamp column index by name.  Pandas writes data
            # columns first and the index column last, so we cannot assume
            # a fixed position.
            ts_col_idx = None
    ...
    # Slow fallback: minimal pandas read
    try:
        df = pd.read_parquet(file_path, columns=["open"])  # minimal read
        if df.empty:
            return None
        return df.index.max().date()
    except Exception as e:
        logger.warning(f"Could not read {file_path}: {e}. Treating as no cache.")
        return None


# ---------------------------------------------------------------------------
# Step 4b: Download in chunks — concurrent with rate limiting
# ---------------------------------------------------------------------------
...
def _download_and_save(
    ...
    """
    Generate date chunks, fetch them from the Upstox API concurrently, and
    batch-write results to Parquet.

    Concurrency strategy:
      - A ThreadPoolExecutor fires API requests in parallel.
      - Each thread acquires a rate-limit token before calling the API,
        ensuring we stay under the per-second and per-minute caps.
      - All chunk DataFrames are collected first, then concatenated and
        written in a single batched I/O pass (one write per Parquet file).
        This eliminates the old per-chunk read-merge-write overhead.
    """
    ...
    def _fetch_task(idx: int, c_start: date, c_end: date) -> tuple[int, pd.DataFrame]:
        """Worker function: rate-limit → fetch → convert to DataFrame."""
        _rate_limit_acquire()
        logger.info(f"  Chunk {idx}/{total}: {c_start} → {c_end}")
        ...
        if raw_candles:
            ...
            logger.info(
                f"  No data returned for chunk {c_start} → {c_end} "
                f"(holiday/weekend range or no trades)."
            )
            return (idx, pd.DataFrame())

    # Use min(MAX_WORKERS, total) threads — no point spawning more than chunks
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in as_completed(futures):
            try:
                ...
                idx = futures[future]
                logger.error(f"  Chunk {idx}/{total} raised an exception: {exc}")

    # ---- Batch write -----------------------------------------------------
    # Combine all non-empty DataFrames, then do a single write pass.
    non_empty = [df for _, df in sorted(chunk_results) if not df.empty]
    if not non_empty:
        logger.info(f"No data returned across all {total} chunks.")
        return

    all_data = pd.concat(non_empty)
    _batch_append_to_parquet(all_data, trading_symbol, storage_unit)
...
def _fetch_one_chunk(
    ...
    Uses the shared API instance created by _get_api_instance() so that
    HTTP connection pooling is reused across all calls.
    ...
    if api_instance is None:
        logger.error(
            "upstox_client is not installed. Historical data download is unavailable "
            "until the Upstox SDK is added to the environment."
        )
        return []
...
def _candles_to_dataframe(candles: list) -> pd.DataFrame:
    ...
    Optimisation: builds columns directly from list comprehensions instead
    of constructing a list-of-dicts, which avoids Python-level per-row
    dict overhead.
    """
    if not candles:
        return pd.DataFrame()

    # Transpose once: column-oriented construction is much faster than
    # row-oriented (list of dicts → DataFrame).
    ...
    ois        = [c[6] for c in candles]

    idx = pd.to_datetime(timestamps, utc=True).tz_convert(IST)
    ...
    df = pd.DataFrame(
        {
            ...
            "oi":     np.array(ois,     dtype=np.int32),
        },
        index=idx,
    )
...
# ---------------------------------------------------------------------------
# Step 4c: Parquet save/append helpers — batched write
# ---------------------------------------------------------------------------

def _batch_append_to_parquet(
    all_df:         pd.DataFrame,
    trading_symbol: str,
    storage_unit:   str,
) -> None:
    """
    Merge all newly downloaded data into the appropriate Parquet file(s)
    on disk in one batched pass (instead of per-chunk read-merge-write).

    For minute data: groups rows by calendar month and writes one file per month.
    For day data: merges into the single instrument file.
    """
    if all_df is None or all_df.empty:
        return

    if storage_unit == "minutes":
        _batch_append_minute_parquet(all_df, trading_symbol)
    elif storage_unit == "days":
        _batch_append_day_parquet(all_df, trading_symbol)


def _batch_append_minute_parquet(df: pd.DataFrame, trading_symbol: str) -> None:
    """
    ...
    # Group by year-month
    ym_labels = df.index.to_period("M")
...
def _batch_append_day_parquet(df: pd.DataFrame, trading_symbol: str) -> None:
    """
