...
thread-safe methods (using threading.RLock).

DESIGN
======
  - No database round-trips — everything lives in RAM for speed.
  - State is reconstructible from logs if the bot crashes.
  - Dashboard reads state directly from here via /api/live/* endpoints.

P1 FIX (2026-04-11) — log_activity missing lock
================================================
``log_activity()`` (the public method) called ``self._log_activity()``
without acquiring ``self._lock``. The private ``_log_activity`` comment
says "must be called while already holding the lock, OR use
log_activity()" — implying the public method should add the lock. It
did not.

``deque.append`` is GIL-safe in CPython for single-threaded appends, but:
  - It is not documented as thread-safe.
  - It is NOT safe in PyPy or future CPython without the GIL.
  - If ``_activity_log`` is ever replaced with a list, the race is real.
  - The inconsistency (internal callers hold lock; public callers don't)
    is a correctness hazard for any future maintainer.

Fix: ``log_activity()`` now acquires ``self._lock`` before calling
``self._log_activity()``. Using ``RLock`` means internal callers that
already hold the lock can still call ``_log_activity()`` directly
without deadlocking.
"""
...
class TickData:
    ...
    candle_volume:   int   = 0
    # Bid/Ask data from marketLevel.bidAskQuote (top level)
...
class LiveState:
    ...
    All public write methods acquire ``self._lock`` (an RLock).
    Internal methods prefixed with ``_`` expect the caller to hold the
    lock — they do NOT acquire it themselves so they can be composed
    without deadlock.
    """

    def __init__(self):
        # RLock allows the same thread to acquire multiple times (re-entrant).
        # This is essential because some public methods call private helpers
        # that may themselves call log_activity.
        ...
        # Rolling activity log — capped at 200 entries
    ...
    def add_position(self, position: LivePosition) -> None:
        ...
        with self._lock:
            if position.symbol in self._positions:
                logger.warning(
                    "[STATE] Position for %s already exists — overwriting.",
                    position.symbol,
                )
            self._positions[position.symbol] = position
            logger.info(
                "[STATE] Position opened: %s %s x%d @ ₹%.2f",
                position.symbol,
    ...
    def update_order_status(
        ...
        with self._lock:
            ...
            if not order:
                logger.warning("[STATE] Order %s not found in open orders.", order_id)
                return None
            order.status = status
    ...
    def record_closed_trade(self, trade: ClosedTrade) -> None:
        with self._lock:
            ...
            logger.info(
                "[STATE] Trade closed: %s %s P&L=₹%.2f (%.2f%%) reason=%s",
    ...
    def debit_cash(self, amount: float) -> None:
        ...
        with self._lock:
            if amount > self._cash:
                raise ValueError(
                    f"Insufficient cash: need ₹{amount:.2f}, have ₹{self._cash:.2f}"
                )
            self._cash -= amount
    ...
    def drawdown_pct(self) -> float:
        ...
        with self._lock:
            ...
            return max(
                0.0,
                (self._peak_capital - self.total_value) / self._peak_capital * 100,
            )
    ...
    def log_activity(self, event_type: str, message: str, level: str = "INFO") -> None:
        """
        Public method for all components to log an activity entry.

        P1 FIX: Now acquires ``self._lock`` before delegating to
        ``_log_activity``.  Using RLock means internal callers that
        already hold the lock (e.g. ``activate_kill_switch``) can still
        call ``_log_activity`` directly without deadlocking.
        """
        with self._lock:
    ...
    def _log_activity(self, event_type: str, message: str, level: str = "INFO") -> None:
        ...
        MUST be called while ``self._lock`` is already held (either
        by the caller or by ``log_activity``).
        """
        entry = {
