...
R2 UPDATE
------------------------
Portfolio tracker — NOT USED by the backtester engine.

P2 FIX (2026-04-11) — dead code quarantine
===========================================
This class is exported from ``backtester/__init__.py`` and appears in
``__all__``, giving the impression it is a functional part of the engine.
It is not. The backtester's ``run_event_loop`` in ``event_loop.py`` manages
cash and positions entirely via plain Python locals — ``Portfolio`` is never
imported, instantiated, or called anywhere in the live codebase.

The class's own docstring (carried over from a previous version) claims it
"is now wired directly into FillEngine and used by run_event_loop". That
statement is false.

This file is kept for backward compatibility (external code may import
``from backtester import Portfolio``) but now raises a
``DeprecationWarning`` on instantiation so those callers are notified.

INTENDED FUTURE WORK
====================
If ``Portfolio`` is ever properly wired into the event loop (replacing the
raw cash/positions locals), the deprecation warning should be removed and
the class restored to active status.

Until then: do not rely on ``Portfolio`` for any production behaviour. The
only authoritative state management is in ``backtester/event_loop.py``.
...
class Portfolio:
    ...
    This class is not used by the backtester engine. The event loop
    (``run_event_loop``) manages cash and positions directly via Python locals.

    Instantiating this class raises a ``DeprecationWarning``.

    See ``backtester/portfolio.py`` module docstring for full context.
    ...
    def __init__(self, config: BacktestConfig) -> None:
        warnings.warn(
            "backtester.Portfolio is dead code — it is not used by the "
            "backtester engine (run_event_loop manages state directly). "
            "This class will be removed in a future release. "
            "Do not rely on Portfolio for production behaviour.",
            DeprecationWarning,
            stacklevel=2,
        )
        self.config   = config
